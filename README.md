...
.\sync-local-docker.ps1 -UseMigrateDev
...
By default the script uses `db:push` (non-interactive). Use `-UseMigrateDev` only when you want `prisma migrate dev` behavior.
