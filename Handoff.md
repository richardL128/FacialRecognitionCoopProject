...
- Enforce exact model/image versions and verify their provenance. `Face_Recognition` is now vendored into this repo as plain files, with its upstream repo and copied-from commit recorded in `Face_Recognition/README.md`. The model weights are still fetched from Hugging Face at runtime and are unpinned; `HF_REPO_ID` in `docker-compose.yml` names a repo but no revision.
...
| Enrolled photos reported missing | Recognition previously treated zero tenant centroids as `not_enrolled`, even when the PIN-verified employee had face-library rows and failed or pending embedding jobs. | Fixed in the working tree on 2026-09-02 with employee-scoped index-state classification and focused tests. Alan Turing's 19 local enrolments were re-indexed successfully; automatic worker retries and alerting remain future hardening. |
